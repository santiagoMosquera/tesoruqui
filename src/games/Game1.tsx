...
export default function Game1({ goBack }: Props): React.JSX.Element {
  ...
  const handleSend = async () => {
    const ok = await sendMessage('1');
    if (ok) Alert.alert('Enviado', 'Se envió el número 1');
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Juego 1</Text>
      <Text style={styles.info}>
        {connectedDevice ? 'Bluetooth conectado' : 'Bluetooth no conectado'}
      </Text>

      <TouchableOpacity style={styles.actionButton} onPress={handleSend}>
        <Text style={styles.buttonText}>Enviar 1</Text>
      </TouchableOpacity>
