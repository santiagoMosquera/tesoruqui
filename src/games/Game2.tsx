...
export default function Game2({ goBack }: Props): React.JSX.Element {
  ...
  const handleSend = async () => {
    const ok = await sendMessage('2');
    if (ok) Alert.alert('Enviado', 'Se envió el número 2');
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Juego 2</Text>
      <Text style={styles.info}>
        {connectedDevice ? 'Bluetooth conectado' : 'Bluetooth no conectado'}
      </Text>

      <TouchableOpacity style={styles.actionButton} onPress={handleSend}>
        <Text style={styles.buttonText}>Enviar 2</Text>
      </TouchableOpacity>
