...
export default function Game4({ goBack }: Props): React.JSX.Element {
  ...
  const handleSend = async () => {
    const ok = await sendMessage('4');
    if (ok) Alert.alert('Enviado', 'Se envió el número 4');
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Juego 4</Text>
      <Text style={styles.info}>
        {connectedDevice ? 'Bluetooth conectado' : 'Bluetooth no conectado'}
      </Text>

      <TouchableOpacity style={styles.actionButton} onPress={handleSend}>
        <Text style={styles.buttonText}>Enviar 4</Text>
      </TouchableOpacity>
